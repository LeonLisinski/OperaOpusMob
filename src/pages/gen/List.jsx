...
import { getDocsLayout, getFilterDefaults, getList, getListItem, getSettings, getStatuses, selectDocs, setProps, setSearchText } from './store';
...
const GenList = (props) => {
	...
	const initLoad = async () => {
		await dispatch(setProps({app:props.match.params.app, module:props.match.params.module}));
			...
			<IonList>
				{list && list.map((item, i) => {
					return <IonItem className='ion-no-padding' button onClick={(e) => onItemClick(e, item)} detail={true} key={i} >
						...
						<IonLabel style={{ paddingLeft: 15 }} className='ion-text-wrap' >
							{
								layouts.glaListItem.map((layoutItem, i) => {
